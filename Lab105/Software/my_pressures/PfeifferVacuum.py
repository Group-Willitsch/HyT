...
class MaxiGauge (object):
    def __init__(self, serialPort, baud=9600, debug=False):
        ...
        self.debug=debug
        try:
            self.connection = serial.Serial(serialPort, baudrate=baud, timeout=10.0)
    ...
    def checkDevice(self):
        ...
        return message + '.isch bein ein berliner'
    ...
    def disco2(self): # claudio 25.04.2019
        try:
...
if __name__ == '__main__':
    ...
    sensor = 1
    reading = mg.send('SPS' , 1)
    print(reading)
    test = mg.send('SP1 [,2,1e-11,1e-5]', 1)
    mg.disco2()