...
#    workerfunc_gui = pyqtSignal(object)
    ...
    def __init__(self):
        ...
        for channel in range(1,8):
            ...
            else:                                                                     #### Temperature: Historical
                exec("self.plot%i.setLabel('left', text='t', units='C')"%(channel))   #### from Arduino
...
#            exec("self.plot%i.getAxis('bottom').setTickSpacing(5, 3)"%(channel))
        ...
        channel = 7 
        exec('temperature = np.array(temperature)')
...
#            print(now)
            ...
            if self.LogMode:
                self.replot_work.emit(self.times,self.timestamps,self.data1,self.data2,self.data3,
                                      self.data4,self.data5,self.data6,self.temperatures) 
            else:
                self.replot_work.emit(self.times,self.timestamps,self.data1,self.data2,self.data3,
                                      self.data4,self.data5,self.data6,self.temperatures) 
