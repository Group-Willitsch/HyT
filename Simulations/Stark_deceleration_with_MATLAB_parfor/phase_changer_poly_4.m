function phase_changer_poly_4(in,param_vec)
arguments
in
param_vec
end
...
%% change phase according to distribution defined here
    ...
    function phase_change = phase_distribution_pol(x)
        ...
        phase_change = a*(x.^4 -(x_0^4)/5) + b*(x.^3 -(x_0^3)/4) + c*(x.^2 -(x_0^2)/2) + d*(x-x_0/2); % divivde by r to set scale of fucntion back start r=100 needed to bring phase cvhange to -4,4 range
