function phase_changer_poly_6(in,param_vec)
arguments
in
param_vec
end
...
if in.params.FLY_focusing_mode_bool
    ...
    for i = 1 : 1 : length(switch_pos_FM)
        ...
        if  any(safer{i,3} <0) || safer{i,1}(end) >= 5e-3
...
% 
%      plot(obj.M_time_vec(end)*1e6,0,'xm')
...
% 
% stairs(M_time_old*10^3,rod1_old,'b')
% stairs(M_time_old*10^3,rod2_old,'b')
% stairs(M_time_old*10^3,rod3_old,'b')
% stairs(M_time_old*10^3,rod4_old,'b')
% 
% ylim([0,9]); xlabel('time(ms)');
% legend('new','new','new','new','old','old', 'old', 'old');
% hold off

in.difference_new_old_t_end = M_time_old(end)-in.M_time_vec(end);
...
%% change phase according to distribution defined here, we start with arctan dist (lower phase beginning, higher towards the end) but can be changed
    ...
    function phase_change = phase_distribution_pol(x)
        ...
        phase_change = a*(x.^6 -(x_0^6)/7) + b*(x.^5 -(x_0^5)/6) + c*(x.^4 -(x_0^4)/5) + d*(x.^3 -(x_0^3)/4) + e*(x.^2 -(x_0^2)/3) + f*(x -(x_0)/2);
    end
